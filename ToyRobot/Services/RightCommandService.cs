...
{
    internal class RightCommandService : ICommandService
    {
        private readonly ILogService _logService;

        public RightCommandService(ILogService logService)
        {
        ...
        public int Process(ref Robot robot, int commandCount)
        {
            if (commandCount == 1)
            {
                string message = "RIGHT command ignore. Cannot do this command on first go";
